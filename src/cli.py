...
class Installer:
    def __init__(self, mode='local', path=None):
        ...
        differ_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ExlMerge.exe').replace('\\', '/')
        self.Exl_Merge = f'{executable_path} {differ_path}'
    ...
    def install(self):
        ...
        self.execute(['diff.xl.command', self.Exl_Merge])
    ...
    def get_git_ignore_path(self):
        ...
        core_excludesfile = self.execute(
            ['--get', 'core.excludesfile']).split('\n')[0]
...
class CommandParser:
    ...
    def execute(self):
        ...
        if not hasattr(self, command):
            return print("Error: Run 'ExlMerge --help' for usage.")
